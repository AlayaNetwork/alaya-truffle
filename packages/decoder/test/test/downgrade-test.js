...
function verifyAbiFunctionDecoding(decoding, address) {
  ...
  let selector = web3.platon.abi.encodeFunctionSignature("causeTrouble()");
...
async function runTestBody(
  ...
  let resultTx = await web3.platon.getTransaction(resultHash);
...
contract("DowngradeTest", function(accounts) {
  ...
  it("Correctly decodes decimals", async function() {
    ...
    const selector = web3.platon.abi.encodeFunctionSignature(
    ...
    let deployedContract = await DowngradeTest.new();
    let address = deployedContract.address;
    let decimalResult = await web3.platon.sendTransaction({
      from: accounts[0],
      to: address,
      data
    });
    ...
    let decimalTx = await web3.platon.getTransaction(decimalHash);
