...
contract("WireTest", _accounts => {
  it("should correctly decode transactions and events", async function() {
    ...
    let constructorTx = await web3.platon.getTransaction(constructorHash);
    let emitStuffTx = await web3.platon.getTransaction(emitStuffHash);
    let moreStuffTx = await web3.platon.getTransaction(moreStuffHash);
    let inheritedTx = await web3.platon.getTransaction(inheritedHash);
    let getterTx1 = await web3.platon.getTransaction(getterHash1);
    let getterTx2 = await web3.platon.getTransaction(getterHash2);
    let defaultConstructorTx = await web3.platon.getTransaction(
