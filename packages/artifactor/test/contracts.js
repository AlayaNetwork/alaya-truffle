...
describe("artifactor + require", () => {
  ...
  before(() => web3.platon.net.getId().then(id => (networkID = id)));
  ...
  before(() =>
    web3.platon.getAccounts().then(_accounts => {
      accounts = _accounts;
    ...
    return Example.new("1", { gas: 3141592 })
      ...
      .then(triggered => {
        ...
        return example.sendTransaction({
          value: web3.utils.toVon("1", "ether")
        });
      })
      .then(
        () =>
          new Promise((accept, reject) =>
            web3.platon.getBalance(example.address, (err, balance) => {
