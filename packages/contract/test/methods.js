...
describe("Methods", function() {
  ...
  describe(".method(): success [ @geth ]", function() {
    ...
    it("should honor the defaultBlock parameter when called", async function() {
      ...
      const initialBlock = await web3.platon.getBlockNumber();
  ...
  describe.skip("web3 wallet", function() {
    it("should work with a web3.accounts.wallet account", async function() {
      ...
      const wallet = web3.platon.accounts.wallet.create(1);
      const providerAccounts = await web3.platon.getAccounts();
      ...
      const balance = await web3.platon.getBalance(wallet["0"].address);
  ...
  describe("sendTransaction() / send() [ @geth ]", function() {
    it("should trigger the fallback function when calling sendTransaction()", async function() {
      ...
      const balance = await web3.platon.getBalance(example.address);
      assert(
    ...
    it("should accept tx params (send)", async function() {
      ...
      const initialSenderBalance = await web3.platon.getBalance(sender);

      await example.send(eth, { from: sender });

      const finalSenderBalance = await web3.platon.getBalance(sender);
      const contractBalance = await web3.platon.getBalance(example.address);
