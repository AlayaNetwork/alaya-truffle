...
describe("Deployments", function() {
  ...
  describe("pre-flight gas estimation", function() {
    ...
    it("should not multiply past the blockLimit", async function() {
      ...
      const block = await web3.platon.getBlock("latest");
  ...
  describe("web3 timeout overrides", function() {
    it("should override 50 blocks err / return a usable instance", async function() {
      ...
      const tempSendTransaction = Example.web3.platon.sendTransaction;
      const tempGetTransactionReceipt = Example.web3.platon.getTransactionReceipt;

      Example.web3.platon.sendTransaction = util.fakeSendTransaction;
      Example.web3.platon.getTransactionReceipt = util.fakeNoReceipt;
      ...
      const example = await Example.new(1).on(
        ...
        async function() {
          ...
          await util.evm_mine();
          await util.evm_mine();
          Example.web3.platon.getTransactionReceipt = util.fakeGotReceipt;
          await util.evm_mine();
        }
      );

      // Restore web3
      Example.web3.platon.sendTransaction = tempSendTransaction;
      Example.web3.platon.getTransactionReceipt = tempGetTransactionReceipt;
