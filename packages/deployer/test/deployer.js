...
describe("Deployer (sync)", function() {
  ...
  beforeEach(async function() {
    ...
    const accounts = await web3.platon.getAccounts();
  ...
  it("waits for confirmations", async function() {
    this.timeout(15000);
    const startBlock = await web3.platon.getBlockNumber();
    ...
    const libReceipt = await web3.platon.getTransactionReceipt(
      IsLibrary.transactionHash
    );
    const exampleReceipt = await web3.platon.getTransactionReceipt(
