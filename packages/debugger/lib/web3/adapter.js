...
export default class Web3Adapter {
  ...
  async getTransaction(txHash) {
    return await this.web3.platon.getTransaction(txHash);
  }

  async getReceipt(txHash) {
    return await this.web3.platon.getTransactionReceipt(txHash);
  }

  async getBlock(blockNumberOrHash) {
    return await this.web3.platon.getBlock(blockNumberOrHash);
  }
  ...
  async getDeployedCode(address, block) {
    ...
    let code = await this.web3.platon.getCode(address, block);
