...
const overrides = {
  ...
  // saying that web3.platon.getBlock is a function and doesn't
  // have a `method` property, which it does
  getBlock: (web3: Web3Shim) => {
    // @ts-ignore
    const _oldBlockFormatter = web3.platon.getBlock.method.outputFormatter;
    // @ts-ignore
    web3.platon.getBlock.method.outputFormatter = (block: any) => {
      ...
      // @ts-ignore
      let result = _oldBlockFormatter.call(web3.platon.getBlock.method, block);
  ...
  getTransaction: (web3: Web3Shim) => {
    const _oldTransactionFormatter =
      // @ts-ignore
      web3.platon.getTransaction.method.outputFormatter;

    // @ts-ignore
    web3.platon.getTransaction.method.outputFormatter = (tx: any) => {
      ...
      let result = _oldTransactionFormatter.call(
        // @ts-ignore
        web3.platon.getTransaction.method,
        tx
      );
  ...
  getTransactionReceipt: (web3: Web3Shim) => {
    const _oldTransactionReceiptFormatter =
      // @ts-ignore
      web3.platon.getTransactionReceipt.method.outputFormatter;

    // @ts-ignore
    web3.platon.getTransactionReceipt.method.outputFormatter = (receipt: any) => {
      ...
      let result = _oldTransactionReceiptFormatter.call(
        // @ts-ignore
        web3.platon.getTransactionReceipt.method,
        receipt
      );
  ...
  decodeParameters: (web3: Web3Shim) => {
    const _oldDecodeParameters = web3.platon.abi.decodeParameters;
    ...
    web3.platon.abi.decodeParameters = (outputs: Array<any>, bytes: String) => {
      ...
      const res = ethersAbiCoder.decode(
        ...
        web3.platon.abi.mapTypes(outputs),
