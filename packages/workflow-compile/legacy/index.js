...
const wasmCompile = require("@truffle/compile-wasm");
...
const Contracts = {
  ...
  compile: async function(options, callback) {
    ...
    try {
      ...
      var compilers = config.compiler
        ? [config.compiler]
        : Object.keys(config.compilers);
      var needReport = true;
      if(compilers.indexOf("wasm") >= 0) {
        compilers = ["wasm"];
	    needReport = false;
      }
  ...
  compileSources: async function(config, compilers) {
	...
	if (compiler !== "wasm" && contracts && Object.keys(contracts).length > 0) {
